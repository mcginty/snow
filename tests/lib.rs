...
fn test_noise_N_legacy() {
        ...
        assert!(h.write_message(&[0u8;0], &mut buffer).unwrap().0 == 48);
...
fn test_noise_X_legacy() {
    ...
    println!("next {}", owner.rng.next_byte);
    let mut h = HandshakeState::new_from_owner(owner,
                        true,
                        HandshakePattern::X,
                        &[0u8; 0],
    ...
    assert!(h.write_message(&[0u8;0], &mut buffer).unwrap().0 == 96);
    //println!("{}", buffer.to_hex());
...
#[test]
fn test_noise_XX_sender_final_tokens() {
    let resolver_i = TestResolver::new(0);
    let resolver_r = TestResolver::new(1);
